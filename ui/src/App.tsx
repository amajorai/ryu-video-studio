...
export function App() {
	...
	useEffect(() => {
		...
		return () => {
			mounted = false;
			for (const url of Object.values(sourcesRef.current)) {
	...
	useEffect(() => {
		const ids = [...new Set(project?.segments.map((s) => s.assetId) ?? [])];
		let canceled = false;
		void (async () => {
			for (const id of ids) {
	...
	}, [project?.id, project?.segments.map((s) => s.assetId).join(","), assets]);
	...
	const organizeAsset = (
		...
		act(async () => {
			...
			const source = sourcesRef.current[asset.id];
			if (source) {
				URL.revokeObjectURL(source);
				delete sourcesRef.current[asset.id];
				setSources({ ...sourcesRef.current });
			}
