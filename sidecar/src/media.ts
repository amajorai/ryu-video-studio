...
export async function probe(
	...
	// child processes reliably. Ask ffprobe to write its bounded JSON result to
	// a private sibling file, then read it through Bun instead of treating a
	// successful empty stdout stream as valid metadata.
	const outputPath = join(
		dirname(path),
		`.ryu-probe-${crypto.randomUUID()}.json`
	);
	try {
		...
		const fileOutput = await Bun.file(outputPath).text();
		return parseProbeOutput(fileOutput || probeOutput);
	} finally {
